...
function newSentence(i){let d=deck();if(!d.length)return;state.index=(state.index+i+d.length)%d.length;state.input='';state.result=null;state.seconds=0;state.hint=0;save();startTimer();render();let c=cur();if(c&&state.question!=='TEXT_ONLY')setTimeout(()=>speak(c.korean,'ko-KR'),250)}
